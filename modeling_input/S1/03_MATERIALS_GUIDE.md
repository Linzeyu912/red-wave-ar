...
用户于 2026-07-20 确认：两张图只用于项目内部研究与抽象建模参考，不放入 App、识图数据库、触发卡或宣传，也不允许完整临摹或重建。
...
上述未知信息不阻塞内部观察；两张图仍不得复制到 `app/`、`modeling_delivery/`、Blender 打包文件、GLB extras、纹理目录或公开演示附件。改变用途前必须重新取得并登记许可。

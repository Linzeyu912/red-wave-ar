...
|---|---|---|---|---:|---|
| S1 | 平西情报联络站 | S1A 入口门楼；S1B 女报务员雕塑 | `S1/00_START_HERE.md` | 14 | 图片已收到；介绍音频文字已定稿；公开权待确认 |
| S2 | 电报大楼 | S2A 电报大楼 | `S2/00_START_HERE.md` | 11 | 图片已收到；介绍音频文字已定稿；公开权待确认 |
| S3 | 短波通信局旧址（北京国际电台中央发信台） | S3A 通信楼；S3B 天线阵列 | `S3/00_START_HERE.md` | 10 | 身份与介绍音频文字已确认；图片公开权待确认 |
| S4 | 居庸关 | S4A 居庸关城楼 | `S4/00_START_HERE.md` | 9 | 图片已收到；介绍音频文字已定稿；公开权待确认 |
| S5 | 西山无名英雄纪念广场 | S5A 纪念雕塑群 | `S5/00_START_HERE.md` | 9 | 介绍音频文字已定稿；内部参考图的水印/权利仍待处理 |
| S6 | 香山镇芳楼 | S6A 镇芳楼 | `S6/00_START_HERE.md` | 9 | 图片已收到；介绍音频文字已定稿；公开权待确认 |
| S7 | 中国电信博物馆 | S7A 博物馆主体 | `S7/00_START_HERE.md` | 7 | 图片已收到；介绍音频文字已定稿；公开权待确认 |
...
4. 单独登记绘制手绘图时使用的真实参考照片，并制作 `<scene>_reference_reveal_vNNN.jpg` 作为内部核对副本；它不用于识别后的照片展示。
...
- **Kivicube 主交付**：每张触发图交付一个目标 ≤5 MB、验收 ≤10 MB 的 `.glb` 与一张 V002 专属地面贴图。七个地点共九套触发图、内部参考图、地面贴图、模型与上传元数据；同一地点的多个模型共用其场景目录和资产卡。

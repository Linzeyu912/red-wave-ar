...
2. 地面边长取模型实际占地长、宽中的较大值，并在四周额外留 `0.10 × 识别图长边`；地面置于 `Y=0.002`。
3. 模型轴心固定在底部中心，最低顶点必须精确接触本地 `Y=0`；场景中模型整体置于 `Y=0.004`，只留避开深度冲突所需的最小间隔。
4. 每张 v002 地面图按模型接触面的材料族制作，并以不超过约 6% 的底材色桥接和软接触阴影处理接缝；阴影只能服务于模型实际占地边缘，不得画成厚展台或黑色底座。
5. 建筑自身的城墙、台阶、雕塑底座和天线基座属于主体，可以保留；禁止额外添加与受控素材无关的厚方台、圆盘展台或大面积不透明地板。
...
| 单模型网格数 | 10 | ≤5 | ≤5 |
| 单模型三角面数 | 50,000 | ≤30,000 | 通常 ≤30,000；S1B 人物特批 ≤50,000 |
| 单模型材质数 | 10 | ≤5 | ≤5 |
| 单模型贴图数 | 10 | 未单列 | ≤10 |
| 整个场景网格数 | 50 | ≤10 | ≤10 |
...
S1B 女报务员属于近景人物识别资产，允许突破 30,000 三角面的官方建议值，但不得突破 50,000 的单模型上限；新增面数必须用于头脸连续曲面、发型、手指、衣褶或设备操作件。上传时仍作为一个完整 GLB，人物、头发、设备和细节只在文件内部按材质合并为少量网格，不拆成多个需要在 Kivicube 中重新对位的独立模型。

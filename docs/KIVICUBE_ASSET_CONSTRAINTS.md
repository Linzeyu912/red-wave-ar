...
每个地点默认采用“手绘触发图—真实参考照片—真实配色模型”的三段式呈现：

1. 用户扫描以红、白为主色的手绘触发图；
2. 识别成功后，先展示绘制该手绘图时使用的真实建筑参考照片；
3. 真实照片淡出或过渡后，再展示按照真实建筑颜色和材质制作的轻量 GLB 模型；
4. 模型稳定出现后开始或继续播放该地点旁白。

默认建议节奏为：真实照片停留 2–3 秒，过渡 0.5–1 秒，模型淡入 0.8–1.2 秒。实际时长可在 Kivicube 场景编辑器内根据真机测试调整。
...
- 每个地点必须完成一次红白手绘图的 Kivicube 星级检查和一次真实印刷品真机识别测试。
...
- “照片—模型”过渡优先在 Kivicube 编辑器内完成，不为了简单淡入额外增加 GLB 骨骼或形变。

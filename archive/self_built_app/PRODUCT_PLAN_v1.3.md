...
> **归档提示**：这是自研 Android / Filament 工作线的历史产品计划，不是当前 LKIVIVUBE 地点素材的建模任务。当前项目入口见仓库根目录 `README.md`，平台交付入口见 `lkivivube_delivery/README.md`。
## 产品计划书（Project Brief）

> **版本**：v1.3（自研程序线历史计划） ｜ **日期**：2026-07-20 ｜ **状态**：2026-07-29 起受项目双线调整约束
> **目标读者**：① 项目负责人 ② 3D 建模协作者 ③ Android 编码协作者 ④ 内容文案与测试人员
> **文档性质**：本文件保留自研 Android / Filament 体验的技术基线与已实现功能。项目级方向以 `docs/PROJECT_DIRECTION.md` 为准，LKIVIVUBE 的模型、图片、旁白和上传物以 `lkivivube_delivery/README.md` 为准；出现冲突时，后两者优先。本文件不得用于声明 LKIVIVUBE 已实现的能力或尚未验收的功能。
...
**实现记录（2026-07-21）**：已完成 `SceneCoordinator`、正式手动场景列表、`scene_id` 端到端传递、Loading 进度/取消、VR 返回/回正/输入模式切换、错误页与诊断页；未知二维码可恢复扫描，加载设 30 秒超时。108 个单元测试、Debug Lint、Debug APK 和模拟器核心导航通过。二维码实扫、陀螺仪方向、碰撞/拾取手感、性能与 20 次进退循环仍须在 nubia Z70 Ultra 真机验收，当前不得标记 M1 真机通过。

...
九个模型均已根据 62 张现有素材重新提取建筑层级、门窗分格、屋檐瓦垄、天线桁架、雕塑衣饰和设备构件，并生成 V3 GLB。2026-08-10 又按多图一致性把细节分为确认、单图、重复画面、年代冲突和推断五类；当前表格是完成本轮证据驱动高精度迭代后的实测结果，见 [`../modeling_input/DETAIL_EVIDENCE_MATRIX.md`](../modeling_input/DETAIL_EVIDENCE_MATRIX.md)。项目上限不是必须用满的面数目标，后续仍只增加有来源且真机可读的细节。GLB 可以保留历史 `photo_emerge` 动画，但静态地面 V002 流程明确不自动播放。生成方法、总览图、摆放参数和机器可读报告见 [`source/README.md`](source/README.md)。

| 单元 | 模型 | 大小 | 网格 | 三角面 | 材质 | 贴图 | 动画 |
|---|---|---:|---:|---:|---:|---:|---:|
| S1A | 平西情报联络站入口门楼 | 375,180 B | 5 | 3,756 | 5 | 1 | 1 |
| S1B | 女报务员雕塑及发报设备 | 2,326,540 B | 4 | 92,660 | 4 | 0 | 1 |
| S2A | 电报大楼 | 884,964 B | 5 | 12,716 | 5 | 0 | 1 |
| S3A | 通信楼 | 549,692 B | 4 | 7,828 | 4 | 0 | 1 |
| S3B | 天线阵列 | 4,242,044 B | 5 | 65,584 | 5 | 0 | 1 |
| S4A | 居庸关城楼 | 585,904 B | 6 | 8,596 | 6 | 1 | 1 |
| S5A | 西山无名英雄纪念广场雕塑群 | 1,327,476 B | 5 | 45,564 | 5 | 1 | 1 |
| S6A | 香山镇芳楼 | 882,644 B | 5 | 13,094 | 5 | 0 | 1 |
| S7A | 中国电信博物馆 | 569,516 B | 5 | 7,616 | 5 | 1 | 1 |

...
- 一个地点交付一个独立 `.glb`；本项目目标为 **≤5 MB**，验收上限为 **≤10 MB**，不设大小下限。
...
| 手绘触发图 | `<scene>_trigger_hand_drawn_vNNN.jpg` | `S2_telegraph_building_trigger_hand_drawn_v001.jpg` |
| 真实照片展示图 | `<scene>_reference_reveal_vNNN.jpg` | `S2_telegraph_building_reference_reveal_v001.jpg` |
...
1. 从 `modeling_input/README.md` 进入对应地点，在 `modeling_input/S?/local_reference/` 放入用户补充的原始图片/文字，并登记手绘触发图、对应真实照片及各自权利状态。

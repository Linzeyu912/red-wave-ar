...
> 状态：`NINE_MODELS_V3_DETAIL_PHOTO_PLANE_READY / BLENDER_5_1_2_REVIEWED / LOCAL_VALIDATION_PASS / PLATFORM_UPLOAD_PENDING`
...
九个模型均已根据 62 张现有素材重新提取建筑层级、门窗分格、屋檐瓦垄、天线桁架、雕塑衣饰和设备构件，并生成 V3 GLB。各文件经 Blender 5.1.2 实际导入、保存可编辑 `.blend` 并渲染预览；GLB 结构和 Kivicube 本地预算检查全部通过。每个 GLB 内置一段 `photo_emerge` 动画，最低点归一到照片平面。生成方法、总览图、摆放参数和机器可读报告见 [`source/README.md`](source/README.md)，逐单元细节依据见 [`../modeling_input/DETAIL_EXTRACTION_V2.md`](../modeling_input/DETAIL_EXTRACTION_V2.md)。

| 单元 | 模型 | 大小 | 网格 | 三角面 | 材质 | 贴图 | 动画 |
|---|---|---:|---:|---:|---:|---:|---:|
| S1A | 平西情报联络站入口门楼 | 295,692 B | 5 | 2,540 | 5 | 1 | 1 |
| S1B | 女报务员雕塑及发报设备 | 261,224 B | 3 | 3,802 | 3 | 0 | 1 |
| S2A | 电报大楼 | 683,368 B | 5 | 9,836 | 5 | 0 | 1 |
| S3A | 通信楼 | 317,896 B | 4 | 4,492 | 4 | 0 | 1 |
| S3B | 天线阵列 | 809,228 B | 3 | 12,476 | 3 | 0 | 1 |

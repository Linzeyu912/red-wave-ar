...
- 手绘触发图：`images/S4_juyong_pass_trigger_hand_drawn_v001.jpg`，评分与印刷真机测试待完成。
- 真实照片展示图：`images/S4_juyong_pass_reference_reveal_v001.jpg`，来源、公开展示许可和隐私检查待完成。
- 模型真实配色依据：待补充，不从红白触发图取色。
- GLB：`model/S4_juyong_pass_v001.glb`，目标 ≤5 MB、验收 ≤10 MB；目标 ≤5 网格、≤30,000 三角面、≤5 材质、≤10 贴图。

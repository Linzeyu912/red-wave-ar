...
## 2026-07-20 ｜ M3D-01R-P 审核通过与冻结 ｜ 移交真机联调

**用户审核结论（通过）**：紧凑地下储洞小室尺度符合小型储洞工作室方向；STYLE-01/02/03 全画幅、无技术烧字；电台控制面朝向正确，面板/旋钮/表窗/线缆在手机构图中可读；`integration_checklist.md` 内容完整，可交编码模型真机联调。适用提交：`d0341c1`。

**冻结**：即日起冻结 M3D-01R-P 的几何、scene.json、GLB、相机构图与风格图，不再做任何呈现层微调；后续任何坐标/起点/交互锚点改动必须随真机联调与编码模型联调确认，并保持 scene.json 为唯一事实源。

**当前闸门（不变）**：不进入 M3D-02/M3D-03、最终高模、最终 PBR、人物或地图。

**下一步**：编码模型依据 `integration_checklist.md` 完成真机联调，重点验证起点朝向、陀螺仪 yaw（WR-1/OQ-I-04）、碰撞、移动边界、拾取与高亮锚点；联调通过并回传结果后，由用户决定是否解锁 M3D-02 核心文物正式建模。

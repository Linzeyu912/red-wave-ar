...
> **版本**：v1.2  ｜ **日期**：2026-07-20  ｜ **作者**：“红色电波”实践团 · 技术组
...
**空间目标**：营造隐蔽、克制、紧张但可清晰参观的地下通信环境。建议空间约 8 m × 8 m，暖色低照度主光，桌面和电台区域有明确引导光，不使用过强烟雾或全屏暗角。

| 资产 ID | 中文名称 | 级别 | 建模与摆放要求 | 交互要求 |
|---|---|---|---|---|
| `env_s1_room` | 地下情报站环境 | MVP | 墙体、地面、门、桌椅、灯具、木箱等合并为少量材质 | 提供外边界和主要障碍物碰撞盒 |
| `p_s1_radio` | 秘密电台 | MVP | 独立 GLB，旋钮、刻度、外壳层次清晰；禁止虚构敏感铭文 | 点击打开介绍并播放讲解 |
| `p_s1_key` | 发报键 | MVP | 独立 GLB，按键臂可选骨骼或节点动画 | 点击可播放短摩尔斯音效；正式解谜为 P2 |
| `p_s1_codebook` | 密码本 | MVP | 合拢状态为基础；翻页或打开动画为 P2 | 点击显示密码通信说明 |
| `p_s1_battery` | 电池/供电设备 | P1 | 结构可信，避免过度细碎线路 | 点击显示供电背景 |
| `d_s1_map` | 历史地图展示面 | P1 | 地图使用获准图片，单独材质，避免 4K 贴图 | 点击放大为 UI 图片，不在 3D 内塞入全文 |

**游客起点建议**：`[0.0, 1.6, 3.0]`，朝向电台桌；首个目标为 `p_s1_radio`。
**MVP 完成条件**：环境、`p_s1_radio`、`p_s1_key`、`p_s1_codebook` 均可加载、拾取、展示文字和播放音频。
...
**动作**：制作 8 m × 8 m 左右白盒，放置起点、桌面、电台、发报键、密码本、墙体和碰撞参考。
...
**动作**：按 `p_s1_radio`、`p_s1_key`、`p_s1_codebook` 顺序制作中模、UV、PBR 材质和运行时 GLB。
